...
const FTC_BASE = "https://apis.data.go.kr/1130000/MllBs_2Service";
const FTC_DETAIL = "https://apis.data.go.kr/1130000/MllBsDtl_3Service";
  ...
  "brands": [
    {
      ...
      "followers": "팔로워 규모 또는 미확인",
      "website": "웹사이트 URL 또는 null",
      "category": "카테고리",
      "sources": ["출처"],
      "description": "브랜드 한줄 설명",
      "aesthetic": "브랜드 감성 2-3단어",
      "popupScore": 7,
      "proposalPoint": "팝업 제안 이유 구체적으로"
    }
  ],
  "summary": "검색 결과 요약"
}
...
const labelStyle = {
  fontSize: 10, letterSpacing: 3, color: "#555", textTransform: "uppercase",
  fontFamily: "monospace", display: "block", marginBottom: 8,
};
const inputBase = {
  width: "100%", background: "#111", border: "1px solid #2A2A2A", borderRadius: 4,
  padding: "11px 14px", color: "#F5F0E8", fontSize: 14, fontFamily: "inherit",
  outline: "none", boxSizing: "border-box",
};
const primaryBtn = (disabled) => ({
  background: disabled ? "#222" : "#F5F0E8",
  color: disabled ? "#555" : "#0A0A0A",
  border: "none", borderRadius: 4, padding: "11px 24px", fontSize: 12,
  fontFamily: "monospace", letterSpacing: 2, cursor: disabled ? "not-allowed" : "pointer",
...
function ErrBox({ msg }) {
  return (
    <div style={{ background: "#1A0A0A", border: "1px solid #3A1A1A", borderRadius: 4, padding: "14px 18px", color: "#EF4444", fontFamily: "monospace", fontSize: 12, marginBottom: 16 }}>{msg}</div>
  );
}
...
export default function BrandScout() {
  ...
  const fetchFTC = async () => {
    ...
    const params = new URLSearchParams({
      serviceKey: API_KEY, pageNo: "1", numOfRows: "50", resultType: "json",
      pBgnDe: nDaysAgo(parseInt(days)), pEndDe: formatDate(today),
      ...(region !== "전체" && { pSidoNm: region }),
    });
    try {
      const res = await fetch(`${FTC_BASE}/getMllBsPYmdInfo_2?${params}`);
      const data = await res.json();
      const items = data?.response?.body?.items?.item;
      const total = data?.response?.body?.totalCount || 0;
      setFtcTotal(total);
      if (!items) { setFtcError("조회 결과가 없습니다."); return; }
      setFtcResults(Array.isArray(items) ? items : [items]);
    } catch (e) { setFtcError("API 오류: " + e.message); }
    finally { setFtcLoading(false); }
  };

  const fetchDetail = async (bizNo) => {
    if (!bizNo) return;
    setDetailLoading(true); setSelectedBiz(null);
    const params = new URLSearchParams({
      serviceKey: API_KEY, pageNo: "1", numOfRows: "1", resultType: "json", pBizNo: bizNo,
    });
    try {
      const res = await fetch(`${FTC_DETAIL}/getMllBsInfoDetail_3?${params}`);
      const data = await res.json();
      const item = data?.response?.body?.items?.item;
      if (item) setSelectedBiz(Array.isArray(item) ? item[0] : item);
    } catch {}
    setDetailLoading(false);
  };

  const toggleSource = (src) => setSelectedSources(prev =>
...
실제 존재하는 브랜드 위주로 인스타그램, 와디즈, 중소벤처기업부/소상공인 지원사업 선정 브랜드 등을 종합해 분석해주세요.`;
    try {
      ...
      setBrands(parsed.brands || []); setSummary(parsed.summary || "");
          ...
          <div>
            ...
            {ftcResults.length > 0 && (
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: selectedBiz || detailLoading ? "1fr 280px" : "1fr", gap: 12 }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {ftcResults.map((item, i) => {
                      const bizNo = item.pBizNo || item.bizNo;
                      const name = item.pBizNm || item.bizNm || "상호명 없음";
                      const rd = item.pRgstDe || "";
                      const date = rd.length === 8 ? `${rd.slice(0,4)}.${rd.slice(4,6)}.${rd.slice(6)}` : rd;
                      return (
                        <div key={i} onClick={() => fetchDetail(bizNo)} style={{ background: "#111", border: "1px solid #1E1E1E", borderRadius: 5, padding: "13px 16px", cursor: "pointer", transition: "border-color 0.15s" }} onMouseEnter={e => e.currentTarget.style.borderColor="#333"} onMouseLeave={e => e.currentTarget.style.borderColor="#1E1E1E"}>
                  ...
                  {(selectedBiz || detailLoading) && (
                    <div style={{ background: "#111", border: "1px solid #2A2A2A", borderRadius: 6, padding: "18px", height: "fit-content", position: "sticky", top: 120 }}>
                      {detailLoading ? <div style={{ textAlign: "center", padding: "30px 0", color: "#444", fontFamily: "monospace", fontSize: 12 }}>조회 중...</div> : selectedBiz && (
                        <div>
                          <div style={{ fontSize: 15, color: "#F5F0E8", marginBottom: 14, paddingBottom: 12, borderBottom: "1px solid #1A1A1A" }}>{selectedBiz.pBizNm || selectedBiz.bizNm}</div>
                          {[["통신판매업번호",selectedBiz.pMllBsNo],["사업자등록번호",selectedBiz.pBizNo],["대표자명",selectedBiz.pRprsntvNm],["사업장주소",selectedBiz.pBplcAddr],["신고일",selectedBiz.pRgstDe],["운영상태",selectedBiz.pBsStCd],["판매방식",selectedBiz.pSlngMthdCd],["취급품목",selectedBiz.pHndlItm]].filter(([,v])=>v).map(([label,val])=>(
